...
export default function CustomerDetailPage() {
  ...
  if (loading || !customer) {
    return <div className="space-y-4">{Array.from({ length: 3 }).map((_, i) => <div key={i} className="h-24 skeleton rounded-[14px]" />)}</div>
  }
  ...
  return (
    <div className="space-y-5 fade-in">
      <PageHeader
        title={customer.name}
        breadcrumb={[{ label: 'Customers', href: '/app/customers' }, { label: customer.name }]}
      />
            ...
            <div className="pt-4 space-y-3">
              ...
              {customer.email && (
                <a href={`mailto:${customer.email}`} className="flex items-center gap-3 text-[13.5px] text-slate hover:text-ink">
                  <Mail size={15} className="shrink-0" />{customer.email}
                </a>
              )}
              {customer.location && (
                <div className="flex items-center gap-3 text-[13.5px] text-slate">
                  <MapPin size={15} className="shrink-0" />{customer.location}
                </div>
              )}
            </div>
            {customer.notes && (
              <div className="mt-4 pt-4 border-t border-sand">
                <p className="text-[12px] font-semibold text-slate uppercase tracking-wide mb-2">Notes</p>
                <p className="text-[13.5px] text-ink">{customer.notes}</p>
              </div>
            )}
          </div>
          ...
          <div className="bg-white border border-sand rounded-[14px] p-5">
            ...
            {orders.length === 0 ? (
              <div className="py-10 text-center">
                <ShoppingBag size={28} className="text-slate mx-auto mb-3" />
                <p className="text-[14px] text-slate">No orders yet</p>
              </div>
            ) : (
